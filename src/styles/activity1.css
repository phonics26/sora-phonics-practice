  ...
  grid.innerHTML = shuffledLetters
    .map(
      (letter) => `
        ...
        >
          <span class="smash-glow"></span>
          <span class="smash-letter">${letter}</span>
          <span class="smash-portal"></span>
        </button>
      `
    )
    .join('')
...
function checkAnswer(button) {
  ...
  const mascot = document.querySelector('#smash-mascot')
  ...
  if (selectedLetter === correctLetter) {
    ...
    mascot.classList.add('smash-celebrate')

    document.querySelector('#smash-score-value').textContent =
      `${score} / ${totalRounds}`

    setTimeout(() => {
      mascot.classList.remove('smash-celebrate')
      currentRound += 1
    ...
    coachMessage.textContent = 'You can do it! Try again.'
  ...
  document.querySelector('#app').innerHTML = `
    <main class="smash-page">
      <section class="smash-results">
        ...
        <p>
          You earned ${score} stars toward your SORA English reward.
        </p>
